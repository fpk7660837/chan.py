    ...
    try:
        for chan_snapshot in chan.step_load():
            ...
            for confirmed in confirmed:
                ...
                elif not confirmed['bsp'].is_buy and position > 0:
                    profit_rate = (current_price - cost_price) / cost_price
                    if profit_rate > 0.03:
                        sell_value = position * current_price * 0.999
                        cash += sell_value
...
def main():
    print("="*80)
    print("缠论买卖点策略 - 简化版稳定买卖点回测".center(80))
    print("="*80)
    ...
    def create_chan_config():
        return {
            ...
            "divergence_rate": 0.9,
            "min_zs_cnt": 1,
            "bs_type": "1,1p,2,2s",  # 需要指定计算哪些类型的买卖点
        }
    ...
    # 只做一类买卖点 + 不同确认周期
    for confirm_klines in [1, 3, 5, 10]:
        test_configs.append({
            "name": f"T1+确认{confirm_klines}K",
            "bsp_filter_config": {"only_t1": True, "need_confirm": confirm_klines},
    ...
    # 保存结果
    json_path = "/Users/fupengkai/Documents/quant/chan.py/backtest_simple_stable_results.json"
