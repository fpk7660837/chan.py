"""
缠论买卖点策略 - 最终优化版 (专注提高准确率)

核心改进:
1. 使用更多类型的买卖点 (1, 1p, 2, 2s, 3)
2. 只在盈利时按卖点卖出
3. 严格止损保护
4. 详细的准确率分析
5. 找出最佳配置组合
...
def calculate_bsp_accuracy_final(trades):
    """
    最终的买卖点准确率计算
    ...
    """
    if not trades:
        return {'overall_accuracy': 0, 'buy_accuracy': 0, 'sell_accuracy': 0, 'details': '无交易'}
    ...
    for i, trade in enumerate(trades):
        if trade['type'] == 'buy':
            ...
            if next_sell:
                ...
                if '止盈' in reason or profit_rate > 0.20:
                    level = 'excellent'
                    score = 1.0
    ...
    return {
        ...
        'buy_excellent': buy_excellent,
        'buy_good': buy_good,
        'buy_fair': buy_fair,
        'sell_excellent': sell_excellent,
        'sell_good': sell_good,
        'sell_fair': sell_fair,
        'buy_positive': buy_excellent + buy_good + buy_fair,
        'sell_positive': sell_excellent + sell_good + sell_fair,
    }


def run_backtest_final(stock_code, stock_name, chan_config_dict, strategy_params):
    """运行最终优化的回测"""
    ...
    try:
        for chan_snapshot in chan.step_load():
            ...
            # 跳过刚处理过的买卖点
            if last_bsp_time == str(last_bsp.klu.time):
                continue
            ...
            if last_bsp.is_buy and position == 0:
                ...
                if buy_volume > 0 and cash >= buy_volume * current_price:
                    ...
                    if cash >= cost:
                        ...
                        last_bsp_time = str(last_bsp.klu.time)
    ...
    accuracy_stats = calculate_bsp_accuracy_final(trades)
...
def main():
    ...
    print("="*80)
    print("缠论买卖点策略 - 最终优化版回测 (专注提高准确率)".center(80))
    print("="*80)
    ...
    for r in successful_results[:20]:
        ...
        summary_data.append({
            ...
            '交易': m['trade_count'],
            '盈利': a['buy_positive'] + a['sell_positive'],
        })
    ...
    report_content = f"""# 缠论买卖点策略 - 最终优化版回测报告
...
- **盈利信号数**: {best_by_accuracy['accuracy']['buy_positive'] + best_by_accuracy['accuracy']['sell_positive']}
...
| 排名 | 股票 | 配置 | 准确率 | 买点准确 | 卖点准确 | 年化收益 | 胜率 | 交易 | 盈利 |
|------|------|------|--------|---------|---------|---------|------|------|------|
"""
    ...
    for i, r in enumerate(successful_results[:10], 1):
        m = r['metrics']
        a = r['accuracy']
        report_content += f"| {i} | {r['stock_name']} | {r['config_name']} | {a['overall_accuracy']*100:.0f}% | {a['buy_accuracy']*100:.0f}% | {a['sell_accuracy']*100:.0f}% | {m['annual_return']*100:6.1f}% | {m['win_rate']*100:.0f}% | {m['trade_count']} | {a['buy_positive'] + a['sell_positive']} |\n"
...
- 高准确率(>40%): {sum(1 for r in successful_results if r['accuracy']['overall_accuracy'] > 0.4)}个配置
- 中准确率(20-40%): {sum(1 for r in successful_results if 0.2 <= r['accuracy']['overall_accuracy'] <= 0.4)}个配置
- 低准确率(<20%): {sum(1 for r in successful_results if r['accuracy']['overall_accuracy'] < 0.2)}个配置
...
"""
    ...
    # 保存报告
    report_path = "/Users/fupengkai/Documents/quant/chan.py/缠论买卖点策略最终优化版回测报告.md"
    ...
    # 保存JSON
    json_path = "/Users/fupengkai/Documents/quant/chan.py/backtest_final_results.json"
    ...
    print(f"\n{'='*80}")
    print("✅ 缠论买卖点策略最终优化版回测完成！".center(80))
    print(f"{'='*80}\n")
