...
def get_weekly_trend_by_bi(weekly_chan):
    """基于周线笔判断趋势 - 更可靠"""
    try:
        if len(weekly_chan) < 3:
            return "NEUTRAL"

        # 获取周线笔列表
        if not hasattr(weekly_chan, 'bi_list') or len(weekly_chan.bi_list) < 2:
            # 没有笔，使用价格判断
            closes = [weekly_chan[i].close for i in range(len(weekly_chan))]
            if len(closes) >= 10:
                recent = sum(closes[-3:]) / 3
                prev = sum(closes[-10:-7]) / 3
                if recent > prev * 1.02:
                    return "UP"
                elif recent < prev * 0.98:
                    return "DOWN"
            return "NEUTRAL"

        bi_list = weekly_chan.bi_list
        last_bi = bi_list[-1]
        prev_bi = bi_list[-2]
        ...
        if not last_bi.is_down:  # 向上笔
            # 检查是否创新高
            if prev_bi:
                if last_bi.end_klc.high >= prev_bi.end_klc.high * 0.98:  # 接近或超过前高
                    return "UP"
            return "UP"  # 最新向上笔，倾向看多
        else:  # 向下笔
            if prev_bi:
                if last_bi.end_klc.low <= prev_bi.end_klc.low * 1.02:  # 接近或超过前低
                    return "DOWN"
            return "DOWN"  # 最新向下笔，倾向看空
...
def get_weekly_bsp_phase(weekly_chan):
    """
    获取周线所处的阶段
    - RISING: 上涨阶段（最近周线有买点或向上笔）
    - FALLING: 下跌阶段（最近周线有卖点或向下笔）
    - CONSOLIDATING: 盘整阶段
    """
    try:
        if len(weekly_chan) < 5:
            return "NEUTRAL"

        # 方法1：检查周线笔
        if hasattr(weekly_chan, 'bi_list') and len(weekly_chan.bi_list) >= 2:
            bi_list = weekly_chan.bi_list
            last_bi = bi_list[-1]

            if not last_bi.is_down:
                return "RISING"  # 最新向上笔
            else:
                return "FALLING"  # 最新向下笔
...
def check_weekly_bsp_resonance(weekly_chan, daily_bsp_is_buy):
    """
    检查周线和日线买卖点是否共振
    - 日线买点 + 周线上升阶段 = 共振
    - 日线卖点 + 周线下跌阶段 = 共振
    """
    try:
        weekly_phase = get_weekly_bsp_phase(weekly_chan)

        if daily_bsp_is_buy:
            # 日线买点：周线应该处于上升或盘整阶段
            return weekly_phase in ["RISING", "NEUTRAL"]
        else:
            # 日线卖点：周线应该处于下跌或盘整阶段
            return weekly_phase in ["FALLING", "NEUTRAL"]
...
def run_backtest_true_resonance(stock_code, stock_name, chan_config_dict, strategy_params, ml_config):
    """运行真正的多级别共振回测"""
    ...
    use_weekly_bsp_resonance = ml_config.get('use_weekly_bsp_resonance', False)
    ...
    # 预加载周线数据到字典，方便按时间查找
    weekly_data_by_time = {}
    for weekly_snapshot in chan_weekly.step_load():
        weekly_chan = weekly_snapshot[0]
        if len(weekly_chan) > 0:
            # weekly_chan是CKLine_List，weekly_chan[-1]是CKLine
            # CKLine有kl_list属性，其中包含CKLine_Unit
            if hasattr(weekly_chan[-1], 'kl_list') and len(weekly_chan[-1].kl_list) > 0:
                last_klu = weekly_chan[-1].kl_list[-1]
                last_time = str(last_klu.time)
                weekly_data_by_time[last_time] = weekly_chan

    # 按时间排序
    sorted_weekly_times = sorted(weekly_data_by_time.keys())
    ...
    try:
        for daily_snapshot in chan_daily.step_load():
            ...
            # 找到当前日线时间对应的周线数据
            weekly_chan = None
            for weekly_time in sorted_weekly_times:
                if weekly_time >= current_time:
                    # 找到第一个晚于或等于当前日线时间的周线
                    weekly_chan = weekly_data_by_time[weekly_time]
                    break

            if not weekly_chan:
                # 如果没有找到，使用最新的周线数据
                if sorted_weekly_times:
                    weekly_chan = weekly_data_by_time[sorted_weekly_times[-1]]

            # 获取周线趋势
            weekly_trend = "NEUTRAL"
            weekly_resonance_ok = True

            if weekly_chan and use_weekly_trend:
                if use_weekly_bsp_resonance:
                    # 使用共振模式
                    weekly_trend = get_weekly_trend_by_bi(weekly_chan)
                else:
                    # 使用简单的趋势判断
                    weekly_trend = get_weekly_trend_by_bi(weekly_chan)
            ...
            if daily_bsp_list:
                for bsp in daily_bsp_list:
                    ...
                    # 周线趋势过滤
                    if weekly_chan and use_weekly_trend and weekly_trend != "NEUTRAL":
                    ...
                    # 周线买卖点共振检查
                    if weekly_chan and use_weekly_bsp_resonance:
                        if not check_weekly_bsp_resonance(weekly_chan, bsp.is_buy):
                            continue
                ...
                if bsp.is_buy and position == 0:
                    ...
                    if buy_volume > 0 and cash >= buy_volume * current_price:
                        ...
                        if cash >= cost:
                            ...
                            if use_weekly_trend:
                                trend_cn = {"UP": "上升", "DOWN": "下跌", "NEUTRAL": "中性"}.get(confirmed.get('weekly_trend', 'NEUTRAL'), "中性")
                                reason += f"[周{trend_cn}]"
...
def main():
    print("="*80)
    print("缠论买卖点策略 - 真正的多级别共振回测".center(80))
    print("="*80)
    ...
    test_configs = [
        {
            ...
            "ml_config": {
                ...
                "use_weekly_bsp_resonance": False,
            },
        ...
        },
        {
            ...
            "ml_config": {
                ...
                "need_confirm": 3,
                "use_weekly_trend": True,
                "use_weekly_bsp_resonance": True,  # 使用共振模式
            },
            "chan_config_fn": create_chan_config,
            "strategy_params": strategy_params_base.copy(),
        },
    ...
    for stock in STOCK_LIST:
        for config in test_configs:
            ...
            result = run_backtest_true_resonance(
    ...
    # 保存结果
    json_path = "/Users/fupengkai/Documents/quant/chan.py/backtest_true_resonance_results.json"
