"""
缠论买卖点策略 - 30分钟+日线多级别共振回测

策略逻辑:
1. 日线趋势向上时，只在30分钟做一类买点
2. 日线趋势向下时，只在30分钟做一类卖点
3. 30分钟买卖点需要确认（3-5根30分钟K线）
"""
...
def get_daily_trend(daily_chan):
    """获取日线趋势"""
    try:
        if len(daily_chan) < 5:
            return "NEUTRAL"
        ...
        # 基于笔判断趋势
        if hasattr(daily_chan, 'bi_list') and len(daily_chan.bi_list) >= 2:
            bi_list = daily_chan.bi_list
            last_bi = bi_list[-1]
            prev_bi = bi_list[-2]

            if not last_bi.is_down:  # 向上笔
                return "UP"
            else:  # 向下笔
                return "DOWN"

        # 基于价格判断
        closes = [daily_chan[i].close for i in range(len(daily_chan))]
        if len(closes) >= 10:
            ma5 = sum(closes[-5:]) / 5
...
def check_daily_30m_resonance(daily_chan, min30_bsp_is_buy):
    """
    检查日线和30分钟的共振
    - 30分钟买点 + 日线上升阶段 = 共振
    - 30分钟卖点 + 日线下降阶段 = 共振
    """
    try:
        daily_trend = get_daily_trend(daily_chan)

        if min30_bsp_is_buy:
            # 30分钟买点：日线应该上升
            return daily_trend in ["UP", "NEUTRAL"]
        else:
            # 30分钟卖点：日线应该下降
            return daily_trend in ["DOWN", "NEUTRAL"]
...
def run_backtest_30m_daily(stock_code, stock_name, chan_config_dict, strategy_params, ml_config):
    """运行30分钟+日线多级别回测"""
    ...
    need_confirm_klines = ml_config.get('need_confirm_klines', 3)  # 30分钟K线数
    use_daily_trend = ml_config.get('use_daily_trend', True)
    use_resonance = ml_config.get('use_resonance', False)
    ...
    for daily_snapshot in chan_daily.step_load():
        ...
        if len(daily_chan) > 0 and hasattr(daily_chan[-1], 'kl_list'):
            last_klu = daily_chan[-1].kl_list[-1]
            daily_time_str = str(last_klu.time)
            # 只保存日期部分（YYYY-MM-DD），用于匹配
            daily_date = daily_time_str.split(' ')[0]
            if daily_date not in daily_data_by_time:
                daily_data_by_time[daily_date] = []
            daily_data_by_time[daily_date].append({
                'time': daily_time_str,
                'chan': daily_chan,
            })

    # 按日期排序
    sorted_daily_dates = sorted(daily_data_by_time.keys())

    try:
        min30_bar_count = 0
        for min30_snapshot in chan_30m.step_load():
            min30_chan = min30_snapshot[0]

            if len(min30_chan) < 2:
                continue

            current_price = min30_chan[-1][-1].close
            current_time = str(min30_chan[-1][-1].time)
            current_kline_idx = min30_chan[-1].idx
            min30_bar_count += 1
            ...
            daily_chan_list = daily_data_by_time.get(current_date, [])

            # 获取日线趋势
            daily_trend = "NEUTRAL"
            daily_resonance_ok = True

            if daily_chan_list and use_daily_trend:
                # 使用当天最新的日线数据
                latest_daily = daily_chan_list[-1]['chan']
                daily_trend = get_daily_trend(latest_daily)

            # 获取30分钟买卖点
            min30_bsp_list = min30_snapshot.get_latest_bsp(number=10)

            # 检查新的买卖点
            if min30_bsp_list:
                for bsp in min30_bsp_list:
                    ...
                    # 日线趋势过滤
                    if use_daily_trend and daily_trend != "NEUTRAL":
                        if bsp.is_buy and daily_trend != "UP":
                            continue
                        if not bsp.is_buy and daily_trend != "DOWN":
                            continue

                    # 日线-30分钟共振检查
                    if use_resonance and daily_chan_list:
                        latest_daily = daily_chan_list[-1]['chan']
                        if not check_daily_30m_resonance(latest_daily, bsp.is_buy):
                            continue
            ...
            for pending in pending_bsps:
                ...
                if klines_passed >= need_confirm_klines:
                    still_valid = False
                    if min30_bsp_list:
                        for current_bsp in min30_bsp_list:
                ...
                if bsp.is_buy and position == 0:
                    ...
                    if buy_volume > 0 and cash >= buy_volume * current_price:
                        ...
                        if cash >= cost:
                            ...
                            if use_daily_trend:
                                trend_cn = {"UP": "上升", "DOWN": "下降", "NEUTRAL": "中性"}.get(confirmed.get('daily_trend', 'NEUTRAL'), "中性")
                                reason += f"[日{trend_cn}]"
    ...
    # 最终清仓
    if position > 0 and len(min30_chan) > 0:
        final_price = min30_chan[-1][-1].close
...
def main():
    print("="*80)
    print("缠论买卖点策略 - 30分钟+日线多级别共振回测".center(80))
    print("="*80)
    ...
    strategy_params_base = {
        "buy_percent": 0.25,
        "stop_loss": -0.05,
        "take_profit": 0.15,  # 30分钟级别，止盈降低
        "bsp_types": [BSP_TYPE.T1, BSP_TYPE.T1P],
    }

    # 测试不同的配置组合
    for need_confirm in [3, 5, 10]:
        test_configs.append({
            "name": f"30m+日趋势+确认{need_confirm}",
            "ml_config": {
                "only_t1": True,
                "need_confirm_klines": need_confirm,
    ...
    for need_confirm in [3, 5]:
        test_configs.append({
            "name": f"30m+日共振+确认{need_confirm}",
            "ml_config": {
                "only_t1": True,
                "need_confirm_klines": need_confirm,
                "use_daily_trend": True,
    ...
    print(f"\n数据源: BaoStock | 回测区间: 2022-2023 (2年)")
    print(f"级别: 30分钟(K_30M) + 日线(K_DAY)")
    ...
    # 保存结果
    json_path = "/Users/fupengkai/Documents/quant/chan.py/backtest_30m_daily_results.json"
    ...
    print(f"\n✅ 详细结果已保存至: {json_path}")
    print(f"\n{'='*80}")
    print("✅ 30分钟+日线多级别共振回测完成！".center(80))
    print(f"{'='*80}\n")
