...
def get_weekly_trend(weekly_chan):
    """获取周线趋势"""
    try:
        ...
        if len(closes) >= 5:
            ...
            if recent_avg > prev_avg * 1.02:  # 上涨超过2%
                return "UP"
            elif recent_avg < prev_avg * 0.98:  # 下跌超过2%
                return "DOWN"
...
def get_weekly_trend_by_ma(weekly_chan, fast=5, slow=10):
    """使用均线判断周线趋势 - 更稳定"""
    try:
        if len(weekly_chan) < slow:
            return "NEUTRAL"
...
def run_backtest_multilevel(stock_code, stock_name, chan_config_dict, strategy_params, ml_config):
    """运行多级别共振回测"""
    ...
    weekly_ma_fast = ml_config.get('weekly_ma_fast', 5)
    weekly_ma_slow = ml_config.get('weekly_ma_slow', 10)
    ...
    try:
        for chan_snapshot in chan.step_load():
            # chan_snapshot[0]是周线，[1]是日线
            weekly_chan = chan_snapshot[0]
            daily_chan = chan_snapshot[1]
            ...
            # 获取周线趋势
            weekly_trend = get_weekly_trend_by_ma(weekly_chan, weekly_ma_fast, weekly_ma_slow) if use_weekly_trend else "NEUTRAL"

            # 获取日线买卖点
            daily_bsp_list = daily_chan.get_latest_bsp(number=10)
...
def main():
    print("="*80)
    print("缠论买卖点策略 - 多级别共振回测".center(80))
    print("="*80)
    ...
    print(f"\n数据源: BaoStock | 时间周期: 周线+日线 | 回测区间: 2018-2023")
    ...
    for stock in STOCK_LIST:
        for config in test_configs:
            ...
            result = run_backtest_multilevel(
    ...
    # 保存结果
    json_path = "/Users/fupengkai/Documents/quant/chan.py/backtest_multilevel_results.json"
